...
## ADR-006: 커밋 전 자동 코드 리뷰 — PreToolUse 훅이 staged diff를 Claude에게 리뷰시킨다

- **Status**: Accepted
- **Date**: 2026-06-04

### Context
ADR-005가 커밋 **후** 정적 가드레일(절대 규칙·문법)을 자동화했지만, 그건 결정론적 텍스트 검사라 **판단이 필요한 리뷰**(설계 냄새, 미묘한 버그, 시크릿/디버그 잔재)는 못 잡는다. 외부 공개 전 코드 리뷰가 TODO로 남아 있었는데(수동 `/code-review`, 자주 빠뜨림), 사람이 기억해야 하는 리뷰는 결국 누락된다. 커밋 **직전**에 강제로 한 번 리뷰를 끼우는 friction이 필요했다.

### Decision
`PreToolUse(Bash)` 훅 `.claude/hooks/pre-commit-review.mjs`를 둔다. git commit을 감지하면 staged diff(`-a`/`--all`이면 `diff HEAD` 보완)를 뽑아 **exit 2로 커밋을 한 번 막고**, 그 출력을 **Claude 컨텍스트로 되돌려** "이 diff를 리뷰하라"고 지시한다(ADR-005의 자기수정 루프와 동일한 exit 2 메커니즘). Claude가 리뷰를 사용자에게 보고하고 동일 커밋을 재실행하면, **diff의 sha1 해시 마커**(임시파일, 30분 만료)가 일치해 통과한다. 무한 루프는 이 마커로 끊는다. 훅 자신의 오류로 커밋을 막지 않도록 **fail-open**(예외 시 exit 0). 의존성 0(Node 내장만).

이 결정은 **ADR-001/005를 supersede하지 않는다** — 앱은 여전히 단일 `index.html`·의존성 0이고, 훅도 Node 내장만 쓴다. commit을 사이에 두고 ADR-005(사후 가드레일)와 짝을 이룬다: **사전 리뷰 → commit → 사후 가드레일**.

### Consequences
- 👍 모든 커밋이 LLM 코드 리뷰를 한 번 거친다. 정적 가드레일(ADR-005)과 상호보완 — 판단형 리뷰(전) + 결정론 검사(후).
- 👍 의존성 0 유지. **fail-open**이라 훅 장애가 작업을 멈추지 않는다(친절한 friction, 게이트키퍼 장애점 아님).
- 👎 리뷰 품질이 Claude 판단에 의존(비결정론). 커밋마다 왕복(차단→리뷰→재커밋)이 한 번 더 생겨 약간 느리다.
- 👎 마커가 해시·시간 기반이라, 리뷰 후 동일 diff를 30분 내 **다른 도구로 직접 커밋**하면 리뷰 없이 통과할 수 있다(개인 로컬 도구라 수용).
- 🧭 노출은 `if: "Bash(git *)"`로 넓게, 정확한 commit 판별·diff 선택은 스크립트가(post-commit 훅과 동일 패턴). 세션 시작 시 PreToolUse가 새로 추가됐으므로 `/hooks` 재로드 또는 재시작이 필요할 수 있다(ADR-005와 동일).

### Alternatives considered
- **결정론적 정적 리뷰 스크립트(린트류)**: 빠르고 재현 가능하나 "판단형 리뷰"가 불가하고 역할이 가드레일(ADR-005)과 겹친다. friction의 목적이 "LLM 리뷰 1회 강제"라 기각(보완재로는 가능).
- **`/code-review` 수동 실행**: 자동 friction이 아니라 자주 빠뜨린다 — 본 ADR의 동기.
- **Stop 훅으로 처리**: Stop은 커밋 타이밍과 결합되지 않아 "커밋 직전" 의미가 흐려진다. 기각.
- **PostToolUse로 사후 리뷰**: 이미 커밋된 뒤라 "사전 게이트"가 안 된다. 기각.
